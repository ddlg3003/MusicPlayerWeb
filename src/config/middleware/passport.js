...
async function checkRegisterUsername(req, res, next) {
    ...
    if (!req.body.username.match(validUsername)) {
        await req.flash('error', 'Username does not meet the requirements!');
        ...
        User.findOne(
            ...
            async function (err, user) {
                ...
                if (!user) {
                    ...
                    await req.flash('error', 'Username has existed!');
                    res.redirect('/auth/register');
                }
            },
        );
...
async function checkRegisterEmail(req, res, next) {
    ...
    if (!req.body.email.match(validEmail)) {
        await req.flash('error', 'Wrong Email format!');
        res.redirect('/auth/register');
    } else {
        User.findOne({ email: req.body.email }, async function (err, user) {
            ...
            if (!user) {
                ...
                await req.flash('error', 'Email has existed!');
                res.redirect('/auth/register');
            }
        });
    }
...
async function checkPassword(req, res, next) {
    ...
    if (!req.body.password.match(validPassword)) {
        await req.flash('error', 'Password does not meet the requirement!');
        res.redirect('/auth/register');
    } else if (req.body.password !== req.body.rePassword) {
        await req.flash('error', 'Retype password does not right!');
        res.redirect('/auth/register');
    } else next();
}
